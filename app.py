...
# 1. Configuration & Design System (V2.3 - STRATEGY DEPTH)
...
def order_flow_engine():
    try:
        ...
        for i in range(5, -6, -1):
            ...
            vol = random.randint(1500, 2500) if p == big_bid_price else random.randint(50, 200)
            if is_current: vol = 0
        ...
        setup_triggered = True if total_bid_vol > (total_ask_vol * 2.7) and dxy_is_safe else False

        # --- RENDER ---
        st.markdown(f"""<div class="kz-topbar"><div style="display:flex; align-items:center; gap:16px;"><div style="background:#eab308; color:#000; font-weight:900; padding:6px 10px; border-radius:4px; font-size:14px;">K</div><div><div style="color:#e4e4e7; font-weight:800; font-size:14px; letter-spacing:1px;">KILLZONE <span style="color:#71717a; font-weight:normal;">| V2.3 INTELLIGENCE</span></div></div></div><div class="kz-topbar-item"><span class="kz-label">CAPITAL UNIT</span><span class="kz-val" style="color:#00ff88;">{cap:,.2f} £</span><div style="background:#222; height:3px; width:60px; margin-top:2px;"><div style="background:#00ff88; height:100%; width:{prog}%;"></div></div></div><div class="kz-topbar-item"><span class="kz-label">SPOT GOLD</span><span class="kz-val">${gold:,.2f}</span></div><div class="kz-topbar-item"><span class="kz-label">DXY FILTER</span><span class="kz-val {'text-green' if dxy_is_safe else 'text-red'}">{dxy:.2f}</span></div><div class="kz-topbar-item"><span class="kz-label">ENGINE STATUS</span><span class="kz-val text-gold">ACTIVE</span></div></div>""", unsafe_allow_html=True)

        c1, c2 = st.columns([1.2, 1.8], gap="large")
        with c1:
            st.markdown(f"""<div class="kz-panel"><div class="kz-header">INSTITUTIONAL ZONES</div><div style="display:flex; flex-direction:column; gap:12px;"><div style="display:flex; justify-content:space-between; border-bottom:1px solid #222; padding-bottom:8px;"><div><span style="font-size:9px; color:#71717a;">VAH</span><br><span style="font-family:'JetBrains Mono'; font-weight:bold; color:#e4e4e7;">{vah:.1f}</span></div><div style="text-align:right;"><span style="font-size:9px; color:#ef4444;">RESISTANCE</span></div></div><div style="display:flex; justify-content:space-between; border-bottom:1px solid #222; padding-bottom:8px;"><div><span style="font-size:9px; color:#eab308; font-weight:bold;">POC</span><br><span style="font-family:'JetBrains Mono'; font-weight:900; color:#eab308; font-size:14px;">{poc:.1f}</span></div><div style="text-align:right;"><span style="font-size:9px; color:#eab308;">MAGNET</span></div></div><div style="display:flex; justify-content:space-between;"><div><span style="font-size:9px; color:#71717a;">VAL</span><br><span style="font-family:'JetBrains Mono'; font-weight:bold; color:#e4e4e7;">{val:.1f}</span></div><div style="text-align:right;"><span style="font-size:9px; color:#10b981;">SUPPORT</span></div></div></div></div>""", unsafe_allow_html=True)
            st.markdown(f"""<div class="kz-panel"><div class="kz-header">ENGINE TELEMETRY</div><div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;"><div style="background:#18181b; padding:10px; border-radius:4px; border:1px solid #27272a;"><div style="color:#71717a; font-size:8px; font-weight:bold; margin-bottom:4px;">REQUIRED RATIO</div><div style="color:#e4e4e7; font-size:14px; font-family:'JetBrains Mono'; font-weight:bold;">2.7x</div></div><div style="background:#18181b; padding:10px; border-radius:4px; border:1px solid #27272a;"><div style="color:#71717a; font-size:8px; font-weight:bold; margin-bottom:4px;">IMBALANCE</div><div style="color:#10b981; font-size:14px; font-family:'JetBrains Mono'; font-weight:bold;">{dom_ratio:.2f}x</div></div></div></div>""", unsafe_allow_html=True)
            
            # --- BLOC ABSORPTION ÉTENDU ---
            st.markdown("""
<div class="kz-panel">
<div class="kz-header">📖 LOGIQUE DÉTAILLÉE : ABSORPTION</div>
<div style="font-size:9.5px; color:#888; line-height:1.45;">
<b>1. Mécanique du Flux :</b> Le moteur détecte l'affrontement entre les <i>Mains Faibles</i> (Vendeurs agressifs "au marché") et les <i>Mains Fortes</i> (Acheteurs passifs "limites").
<br><br>
<b>2. Le Mur (Imbalance 2.7x) :</b> Pour que l'absorption soit confirmée, le mur d'ordres acheteurs sur la Heatmap doit être 2.7 fois supérieur à la résistance vendeuse. C'est le point de rupture institutionnel.
<br><br>
<b>3. Psychologie :</b> Le prix "stagne" car les ordres limites absorbent chaque lot vendu. Les vendeurs paniquent car le prix ne descend plus malgré leurs frappes massives. 
<br><br>
<span style="color:#eab308;">●</span> <b>Filtre DXY :</b> Le Dollar est le miroir de l'Or. Si le DXY est en pleine impulsion haussière, l'algorithme bloque l'achat car le mur de liquidité risque d'être transpercé par la macro-économie.
</div>
</div>
""", unsafe_allow_html=True)
        ...
        with c2:
            ...
            if setup_triggered: st.markdown("""<div class="setup-alert"><div style="font-weight:900; font-size:12px; margin-bottom:4px;">ABSORPTION BULLISH DETECTED | RATIO 2.7x</div></div>""", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
            st.markdown("""<div class="kz-panel"><div class="kz-header">🗺️ FEUILLE DE ROUTE & DISCIPLINE</div><div class="roadmap-item"><div class="roadmap-num">1</div><div style="font-size:10px;"><b>LOCALISATION :</b> Prix en contact avec une zone (POC, VAL ou VAH).</div></div><div class="roadmap-item"><div class="roadmap-num">2</div><div style="font-size:10px;"><b>LIQUIDITÉ :</b> Présence d'un mur > 1500 lots sur la Heatmap.</div></div><div class="roadmap-item"><div class="roadmap-num">3</div><div style="font-size:10px;"><b>ALGORITHME :</b> Ratio d'imbalance validé à 2.7x minimum.</div></div><div class="roadmap-item"><div class="roadmap-num">4</div><div style="font-size:10px;"><b>MACRO :</b> Filtre DXY "SAFE" (Dollar ne pompe pas à la hausse).</div></div><div class="roadmap-item"><div class="roadmap-num">5</div><div style="font-size:10px;"><b>SIGNAL :</b> Attendre le déclenchement du badge d'alerte.</div></div></div>""", unsafe_allow_html=True)
    except Exception as e: st.error(f"Engine Offline: {e}")
