...
# 1. Config & Interface Compacte V23/V26
...
if data:
    ...
    # --- LOGIQUE TEMPORELLE ---
    # Hypothèse : 1 palier (doublement) demande environ 12 trades gagnants avec R:R 1:2 à 6% de risque.
    # Rythme : 3 setups validés / semaine. Donc 1 palier ~= 4 semaines (1 mois).
    paliers_restants = math.log(objectif / capital_actuel) / math.log(2)
    mois_estimes = paliers_restants * 1.2 # Marge de sécurité
    ...
    h1.markdown(f"<h3 style='color:#ffb000; margin:0;'>🔱 BEE-INVEST UNIT</h3><small style='color:#444; font-size:8px;'>ROAD TO 1M | RISK 6% | TARGET: {date_cible.strftime('%B %Y')}</small>", unsafe_allow_html=True)
    ...
    with col_main:
        # ROADMAP + TEMPS
        st.markdown("<p class='label'>● ROADMAP & PROJECTION TEMPORELLE</p>", unsafe_allow_html=True)
        st.markdown(f"""
        <div class='roadmap-box'>
            <div style='display:flex; justify-content:space-between; font-size:10px;'>
                <span>Paliers : <b>{paliers_restants:.1f}</b> | Estimation : <b>~{mois_estimes:.1f} mois</b></span>
                <span style='color:#ffb000;'>{progression:.2f}%</span>
            </div>
            <div style='background:#222; height:6px; margin-top:4px;'><div style='background:#ffb000; height:100%; width:{progression}%;'></div></div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with c_ex2:
            st.markdown(f"""<div class='kz-card' style='font-size:11px;'><small class='label'>PALIERS PRIX & SORTIE</small><br>🟢 TP : <b>{gold+20:,.1f}</b> (Retrait 10% si >5k)<br>⚪ IN : <b>{gold:,.1f}</b><br>🔴 SL : <b>{gold-15:,.1f}</b></div>""", unsafe_allow_html=True)

        # DIAGRAMME AVEC TABLEAU PALIERS DÉTAILLÉ
        st.markdown("<p class='label'>● CALENDRIER DES PALIERS ET CAPITALISATION</p>", unsafe_allow_html=True)
        p_list = []
        temp_cap = capital_actuel
        now = datetime.now()
        for i in range(1, 12):
            temp_cap *= 2
            # On estime 1.2 mois par doublement (palier)
            date_p = now + timedelta(days=i*1.2*30)
            p_list.append({
                "Palier": f"P{i:02}", 
                "Capital Target": f"{temp_cap:,.0f} £", 
                "Echéance": date_p.strftime('%m/%Y'),
                "Retrait": f"{(temp_cap*0.1):,.0f} £" if temp_cap > 5000 else "0 £"
            })
        
        # Affichage du tableau de Roadmap
        st.table(pd.DataFrame(p_list).head(10))

        # NEWS (Toujours là, en bas)
