...
# 1. Configuration & Design System (V94 ANTI-FLICKER OMEGA)
...
# CSS CHIRURGICAL : STOPPE LE FLASH NOIR & RESTE SUR LA DOMINANCE
st.markdown("""
<style>
    :root {
...
def sync_terminal():
    try:
        ...
        # --- RENDER ---
        st.markdown(f"""<div style='display:flex; justify-content:space-between;'><div><h3 style='color:#ffb000; margin:0;'>🔱 BEE-INVEST UNIT</h3><small style='color:#444;'>V94 DOMINANCE ENGINE | SOLDE: 953.55 GBP</small></div><div class='val-quant'>{gold:,.2f} $ <span style='padding: 2px 6px; border-radius: 3px; font-size: 9px; font-weight: bold; margin-left: 10px; background:{sig_col}22; color:{sig_col}; border:1px solid {sig_col};'>{sig_label}</span></div></div>""", unsafe_allow_html=True)
        ...
        with c2:
            ...
            <div class="regime-box">
                <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:8px;">
                    ...
                    <div class="regime-card {'active' if regime == 'VOL' else ''}" style="background:#111; border:1px solid {'#ffb000' if regime == 'VOL' else '#1a1a1a'}; border-radius:6px; padding:12px 5px; text-align:center; color:{'#e0e0e0' if regime == 'VOL' else '#444'};">~<br><span>Vol</span></div>
                </div>
            ...
            # NEWS & INTEL
            st.markdown("<p class='label'>● NEWS FEED & INTEL</p>", unsafe_allow_html=True)
            ...
            st.markdown("""
            <div class="intel-box"><div class="intel-title">⚙️ VOLUME PROFILE</div><div class="intel-content">VA = Zone de 70% du volume. Flux institutionnel.</div></div>
            <div class="intel-box"><div class="intel-title">📊 GOLD OUTLOOK</div><div class="intel-content">Bullish: Géopolitique. Bearish: Dollar / Yields.</div></div>
            """, unsafe_allow_html=True)
