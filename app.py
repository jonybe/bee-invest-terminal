...
# 1. Config & Interface Compacte V23
...
if data:
    ...
    with col_main:
        ...
        st.markdown(f"""<div class='roadmap-box'><div style='display:flex; justify-content:space-between; font-size:10px;'><span>Paliers : <b>{paliers_restants:.1f}</b></span><span style='color:#ffb000;'>{progression:.2f}%</span></div><div style='background:#222; height:6px; margin-top:4px;'><div style='background:#ffb000; height:100%; width:{progression}%;'></div></div></div>""", unsafe_allow_html=True)
        ...
        # DIAGRAMME DE CROISSANCE (FIXÉ)
        st.markdown("<p class='label'>● DIAGRAMME DE CROISSANCE COMPOSÉE</p>", unsafe_allow_html=True)
        p_list = []
        temp_cap = capital_actuel
        for i in range(1, 12):
            temp_cap *= 2
            p_list.append({"Palier": f"P{i:02}", "Capital": temp_cap}) # Format P01, P02 pour forcer le tri correct
        df_p = pd.DataFrame(p_list)
        st.line_chart(df_p.set_index("Palier")["Capital"])
