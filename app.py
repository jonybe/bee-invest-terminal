...
# 1. Configuration & Design System (V82 LOCKED)
st.set_page_config(page_title="BEE-INVEST | TOTAL CONTROL", layout="wide")

# Initialisation du gestionnaire de trades (Persistance Streamlit)
...
def sync_terminal():
    try:
        ...
        # 4. TRADES MANAGEMENT (M5 CLOSE TRIGGER)
        # Nettoyage des trades clôturés (TP/SL touchés)
        active_trades = []
        ...
        # Détection nouvelle bougie M5 close
        current_m5_ts = df_m5.index[-1]
        if st.session_state.last_m5_ts != current_m5_ts:
            if len(st.session_state.trades) < 2:
                if sig_label == "ACHAT" and gold <= vah:
                    st.session_state.trades.append({'type': "LONG", 'in': gold, 'tp': gold+24, 'sl': gold-12, 'ts': current_m5_ts})
                elif sig_label == "VENTE" and gold >= val:
                    st.session_state.trades.append({'type': "SHORT", 'in': gold, 'tp': gold-24, 'sl': gold+12, 'ts': current_m5_ts})
            st.session_state.last_m5_ts = current_m5_ts

        # --- RENDER UI ---
        st.markdown(f"""<div style='display:flex; justify-content:space-between;'><div><h3 style='color:#ffb000; margin:0;'>🔱 BEE-INVEST UNIT</h3><small style='color:#444;'>V82 M5 EXECUTION ENGINE | persistent TRADES</small></div><div class='val-quant'>{gold:,.2f} $ <span class='status-tag' style='background:{sig_col}22; color:{sig_col}; border:1px solid {sig_col};'>{sig_label}</span></div></div>""", unsafe_allow_html=True)
        ...
        with c1:
            ...
            for b, v in v_profile.items():
                p_lvl = p_min + (b * (p_max - p_min) / 20)
                fig.add_shape(type="rect", xref="paper", yref="y", x0=0, x1=(v/max_v)*0.15, y0=p_lvl, y1=p_lvl+bin_size, fillcolor="rgba(150,150,150,0.1)", line_width=0)
            ...
            # --- DYNAMIC PERSISTENT HUNTER (MAX 2) ---
            ...
            if st.session_state.trades:
                ...
                for idx, tr in enumerate(st.session_state.trades):
                    b_color = "#00ff88" if tr['type'] == "LONG" else "#ff4b4b"
                    with cols[idx]:
                        st.markdown(f"""<div class='kz-card' style='border-left:3px solid {b_color};'>
                            <div style='display:flex; justify-content:space-between;'><b style='color:{b_color};'>{tr['type']} ACTIVE</b><small style='color:#444;'>{tr['ts'].strftime('%H:%M')}</small></div>
                            <div style='font-family:JetBrains Mono; font-size:10px; margin-top:5px;'>
                                ⚪ IN: {tr['in']:,.2f}<br>🟢 TP: {tr['tp']:,.2f}<br>🔴 SL: {tr['sl']:,.2f}<br>
                                📊 LOTS: {(cap*0.06)/120:.2f}
                            </div>
                        </div>""", unsafe_allow_html=True)
            else:
                st.markdown(f"<div class='kz-card' style='text-align:center; color:#444; padding:20px;'>⌛ WAITING FOR M5 CLOSE SIGNAL...</div>", unsafe_allow_html=True)
            ...
            for i in range(1, 7):
                ...
                st.markdown(f"""<div class="matrix-row"><div class="m-id">P{i:02}</div><div style="width:200px; color:white; font-weight:bold;">{tr_m:,.0f} £ <span style="color:#444; font-weight:normal; font-size:10px;">/ {tr_g:,.0f} £</span></div><div style="color:#00ff88; font-weight:bold;">LOT: {(tr_m*0.06)/120:.2f}</div>{badge}</div>""", unsafe_allow_html=True)
