...
# 1. Configuration & API
st.set_page_config(page_title="KILLZONE | Terminal V6", layout="wide", initial_sidebar_state="collapsed")
...
# CSS INTERFACE (Anti-Flicker & Zero Gap)
st.markdown("""
<style>
    ...
    .main-grid { display: grid; grid-template-columns: 300px 1fr 320px; gap: 10px; padding: 10px; height: 98vh; }
    .kz-card { background: #0d0d0d; border: 1px solid #1a1a1a; border-radius: 4px; padding: 12px; margin-bottom: 10px; }
    ...
    /* DOM STYLING */
    .dom-box { background: #080808; border: 1px solid #1a1a1a; height: 550px; overflow: hidden; border-radius: 4px; }
...
def terminal():
    ...
    # --- TOPBAR ---
    st.markdown(f"""
    <div style="background:#0d0d0d; border-bottom:2px solid #eab308; padding:10px 20px; display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
        <div style="display:flex; align-items:center; gap:20px;"><b style="color:#eab308; font-size:20px;">BEE-INVEST</b><span style="color:#333;">|</span><small>ELITE TERMINAL V6.0</small></div>
    ...
    with c1:
        ...
        <div class="kz-card">
            ...
            <div style="text-align:center; padding:15px; background:#080808; border-radius:4px; border:1px solid #222;">
            ...
            <p style="font-size:8px; color:#444; margin-top:5px;"><b>DEFINITION:</b> Convergence Flow L2 + POC + DXY.</p>
        </div>
        <div class="kz-card">
    ...
    with c2:
        st.markdown("""<div class="kz-card" style="height:500px;"><div class="kz-header">TradingView Elite Analysis (M15)</div>""", unsafe_allow_html=True)
    ...
    with c3:
        st.markdown(f"""<div class="kz-card" style="height:500px;"><div class="kz-header">Order Flow Matrix (L2)</div><div class="dom-box">{dom_html}</div></div>""", unsafe_allow_html=True)

    # --- ROADMAP ---
    st.markdown(f"""
    <div style="background:#0d0d0d; border:1px solid #1a1a1a; padding:12px; border-radius:4px; display:grid; grid-template-columns: repeat(4, 1fr); gap:20px; text-align:center; margin:0 10px;">
