...
def get_realtime_data(symbol):
    """Récupère le prix via Twelve Data API"""
    try:
        url = f"https://api.twelvedata.com/price?symbol={symbol}&apikey={API_KEY}"
        response = requests.get(url).json()
        return float(response['price'])
    except:
        return 2350.0 # Fallback si limite atteinte
...
@st.fragment(run_every=8) # 8s pour respecter les limites Twelve Data Free
def order_flow_engine():
    try:
        # A. LIVE DATA FETCHING
        gold = get_realtime_data("XAU/USD")
        dxy = get_realtime_data("DXY")
        dxy_is_safe = True if dxy < 107.0 else False 
        ...
        for i in range(5, -6, -1):
            ...
            vol = random.randint(1800, 2600) if p == big_bid_price else random.randint(100, 350)
            if is_current: vol = 0
...
<div><div style="color:#e4e4e7; font-weight:800; font-size:14px; letter-spacing:1px;">KILLZONE <span style="color:#71717a; font-weight:normal;">| V2.4 LIVE API</span></div></div>
</div>
...
<div class="kz-topbar-item"><span class="kz-label">SPOT GOLD (TD)</span><span class="kz-val">${gold:,.2f}</span></div>
...
<div class="kz-topbar-item"><span class="kz-label">SYNC</span><span class="kz-val text-gold">STABLE</span></div>
</div>
""", unsafe_allow_html=True)
        ...
        with c1:
            ...
            st.markdown("""<div class="kz-panel"><div class="kz-header">📖 LOGIQUE : ABSORPTION</div><div style="font-size:9.5px; color:#888; line-height:1.45;"><b>Absorption :</b> Affrontement entre <i>Mains Faibles</i> (Vendeurs agressifs) et <i>Mains Fortes</i> (Acheteurs passifs). <br><br><b>Le Mur :</b> Ratio 2.7x confirmé sur zone clé (POC/VAL). <br><br><span style="color:#eab308;">●</span> <b>Filtre DXY :</b> Bloque l'achat si le dollar est en impulsion haussière.</div></div>""", unsafe_allow_html=True)
        ...
        with c2:
            ...
            for row in dom_data:
                ...
                width_pct = min(100, (vol / 2600) * 100)
                if row['type'] == "ask": dom_html += f"<div class='dom-row'><div class='dom-price'>{p}</div><div class='dom-bar-container'><div class='dom-bar-ask' style='width:{width_pct}%;'></div><div class='dom-vol'>{vol}</div></div></div>"
                elif row['type'] == "current": dom_html += f"<div class='dom-row' style='margin: 8px 0; border-top: 1px solid #eab308; border-bottom: 1px solid #eab308;'><div class='dom-price current'>{p}</div><div class='dom-bar-container' style='background:transparent;'><div style='padding-left:8px; color:#eab308; font-size:9px; font-weight:bold; line-height:16px;'>SPOT PRICE (SPREAD)</div></div></div>"
                else: dom_html += f"<div class='dom-row'><div class='dom-price'>{p}</div><div class='dom-bar-container'><div class='dom-bar-bid' style='width:{width_pct}%;'></div><div class='dom-vol'>{vol}</div></div></div>"
            st.markdown(dom_html + "</div>", unsafe_allow_html=True)
            if setup_triggered: st.markdown("""<div class="setup-alert"><div style="font-weight:900; font-size:12px;">ABSORPTION BULLISH DETECTED | RATIO 2.7x</div></div>""", unsafe_allow_html=True)
