...
def order_flow_engine():
    try:
        ...
        t_ask, t_bid = 0, 0
        current_setup = "NONE"
        if gold <= val + 0.8: current_setup = "BULL_ABS"
        elif gold >= vah - 0.8: current_setup = "BEAR_ABS"
        ...
        with c2:
            ...
            fig.add_hline(y=val, line_color="#10b981", line_width=1, opacity=0.2)
            fig.update_layout(margin=dict(l=0,r=0,t=0,b=0), height=180, template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', showlegend=False, xaxis_rangeslider_visible=False, yaxis=dict(gridcolor='#111', side="right"))
        ...
        with c3:
            ...
            for row in dom_data:
                ...
                bar_type = "ask" if row['t'] == "ask" else "bid"
                dom_html += f"""<div class="dom-row"><div class="dom-price" style="{price_style}">{row['p']:.1f}</div><div class="dom-bar-container">{"<div class='dom-bar-"+bar_type+"' style='width:"+str(width)+"%;'></div>" if not is_curr else ""}<div class="dom-vol">{row['v'] if row['v']>0 else ""}</div></div></div>"""
