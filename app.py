...
# 1. Configuration & Design System (V65 LOCKED)
...
def sync_terminal():
    try:
        ...
        # REDUCTION DU DELAY : On compare le prix LIVE au prix d'il y a 2 bougies
        ...
        for n in feed.entries[:20]:
            title = n.title.upper()
            if any(x in title for x in ["PPI", "CPI", "PMI", "FED", "NFP", "RATE"]):
        ...
        with c_main:
            ...
            t1, t2 = st.columns(2)
            t1.markdown(f"<div class='kz-card' style='font-size:11px; border-left:3px solid #ffb000;'>🟢 <b>TP:</b> {gold+30:,.2f}<br>⚪ <b>IN:</b> {gold:,.2f}<br>🔴 <b>SL:</b> {gold-15:,.2f}</div>", unsafe_allow_html=True)
            t2.markdown(f"<div class='kz-card' style='font-size:11px; border-left:3px solid #00ff88;'>💰 <b>GAIN:</b> +115.15 £<br>⚠️ <b>RISQUE:</b> -57.57 £<br>📊 <b>LOT:</b> 0.24</div>", unsafe_allow_html=True)
            ...
            for i in range(1, 6):
                ...
                st.markdown(f"""<div class="matrix-row"><div class="m-id">P{i:02}</div><div style="color:white; font-weight:bold;">{tr:,.0f} £</div><div style="color:#00ff88; font-weight:bold;">L: {(tr*0.06)/150:.2f}</div>{badge}</div>""", unsafe_allow_html=True)

        with c_side:
            # Dominance - POURCENTAGE RÉTABLI ICI
            ...
            st.markdown(f"""<div class="intel-desk-sidebar"><p class='label' style='color:#ffb000;'>⚔️ STRATEGIC INTEL</p><div style="font-size:10px; color:#aaa;"><b>LIVE SYNC :</b> 2s latency.<br><b>IMPULSE :</b> Real-time calculated.</div></div>""", unsafe_allow_html=True)
            ...
            for n in news_list[:3]:
                st.markdown(f"<div style='font-size:10px; border-bottom:1px solid #111; padding:4px 0;'>🕒 {n.published[5:11]} | {n.title[:40]}...</div>", unsafe_allow_html=True)
