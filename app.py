...
# 1. Configuration & Design System (V78 LOCKED)
...
<style>
    ...
    .signal-header { display: flex; justify-content: space-between; margin-bottom: 4px; font-weight: bold; font-family: 'JetBrains Mono'; }
</style>
...
def sync_terminal():
    try:
        ...
        vah, val = poc_price + (vp_data['Close'].std() * 1.1), poc_price - (vp_data['Close'].std() * 1.1)
        ...
        status_text = "NEUTRAL" if sig_label == "ATTENTE" else "BULLISH" if sig_label == "ACHAT" else "BEARISH"

        # 6. RENDER
        st.markdown(f"""<div style='display:flex; justify-content:space-between;'><div><h3 style='color:#ffb000; margin:0;'>🔱 BEE-INVEST UNIT</h3><small style='color:#444;'>V78 MACRO ANALYST | M15 STABLE</small></div><div class='val-quant'>{gold:,.2f} $ <span class='status-tag' style='background:{sig_col}22; color:{sig_col}; border:1px solid {sig_col};'>{status_text}</span></div></div>""", unsafe_allow_html=True)
        ...
        with c1:
            ...
            if opp:
                e, d = gold, 12.0
                t_p, s_p = (e+d*2 if opp=="LONG" else e-d*2), (e-d if opp=="LONG" else e+d)
                sc1, sc2 = st.columns(2)
                sc1.markdown(f"<div class='kz-card' style='border-left:3px solid #00ff88;'>🔥 <b>{opp} SETUP</b><br>⚪ IN: {e:,.2f}<br>🟢 TP: {t_p:,.2f}<br>🔴 SL: {s_p:,.2f}</div>", unsafe_allow_html=True)
        ...
        with c2:
            # --- SIGNAL SCORE HEADER ---
            st.markdown(f"""<div class='signal-header'><span style='color:#555;'>BULL SCORE</span><span style='color:{sig_col};'>{sig_label} {bull_score:.1f}%</span></div>""", unsafe_allow_html=True)
            ...
            st.markdown(f"<div class='macro-note'>Vendre si DXY > 102.50. Chute = Bullish Gold.</div>", unsafe_allow_html=True)
            st.metric("REAL YIELDS", f"{yields:.2f}%")
            st.markdown(f"<div class='macro-note'>Bullish Gold si Yields < 4.0%.</div>", unsafe_allow_html=True)
            st.metric("VIX INDEX", f"{vix:.2f}")
            st.markdown(f"<div class='macro-note'>Safe Haven si VIX > 20. Panique = Achat Gold.</div>", unsafe_allow_html=True)
            
            # Calendar
            st.markdown("<p class='label'>● ECONOMIC CALENDAR (LIVE)</p>", unsafe_allow_html=True)
            st.markdown("<div class='kz-card' style='padding:8px;'><div style='display:flex; font-size:8px; color:#444; margin-bottom:5px;'><span>EVENT</span><span style='margin-left:auto;'>ACT / EXP</span></div>" + 
                "".join([f"<div style='display:flex; font-size:10px; padding:4px 0; border-bottom:1px solid #111;'><span>{ev.title[:5]}</span><span style='margin-left:auto; color:#00ff88;'>--</span></div>" for ev in feed.entries[:2]]) + "</div>", unsafe_allow_html=True)

        st.markdown(f"<div style='background:{sig_col}; color:black; text-align:center; padding:10px; font-weight:900; border-radius:4px; margin-top:10px;'>VERDICT FINAL : {status_text} | {bull_score:.1f}%</div>", unsafe_allow_html=True)
