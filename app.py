...
# 1. Configuration & API
st.set_page_config(page_title="KILLZONE | Terminal V6.2", layout="wide", initial_sidebar_state="collapsed")
...
# 2. CSS RADICAL (Supprime TOUS les espaces Streamlit)
st.markdown("""
<style>
    [data-testid="stAppViewBlockContainer"] { padding: 0.5rem 1rem !important; }
    div[class^="st-emotion-cache"] { gap: 0rem !important; margin: 0 !important; padding: 0 !important; }
    ...
    /* FIX IFRAME POSITION */
    .iframe-container { background: #0d0d0d; border: 1px solid #1a1a1a; border-radius: 4px; padding: 10px; height: 500px; display: flex; flex-direction: column; }
    iframe { width: 100%; flex-grow: 1; border: none !important; border-radius: 4px; margin: 0 !important; }
</style>
...
def terminal():
    ...
    for i in range(14, -15, -1):
        ...
        dom_html += f'<div class="dom-row"><div class="dom-p" style="{p_style}">{p:.1f}</div><div class="dom-bar-wrap">'
        ...
        dom_html += f'<div class="dom-vol">{v if not is_curr else ""}</div></div></div>'
    ...
    <div style="background:#0d0d0d; border-bottom:2px solid #eab308; padding:8px 20px; display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
        <div style="display:flex; align-items:center; gap:15px;"><b style="color:#eab308; font-size:18px;">BEE-INVEST</b><span style="color:#333;">|</span><small>ELITE MATRIX V6.2</small></div>
    ...
    with c1:
        ...
        <div class="kz-card">
            ...
            <div style="text-align:center; padding:10px; background:#080808; border-radius:4px; border:1px solid #222;">
        ...
        <div class="kz-card" style="height:352px;">
            <div class="kz-header">Weekly Calendar</div>
    ...
    with c2:
        # CONTENEUR UNIQUE POUR LE GRAPH (BLOQUE LA FUITE)
        st.markdown('<div class="iframe-container"><div class="kz-header">TradingView Elite (M15)</div>', unsafe_allow_html=True)
    ...
    with c3:
        st.markdown(f"""<div class="kz-card" style="height:500px;"><div class="kz-header">Order Flow Matrix</div><div class="dom-box">{dom_html}</div></div>""", unsafe_allow_html=True)
