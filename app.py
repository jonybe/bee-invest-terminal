...
# 1. Config & Interface Compacte
st.set_page_config(page_title="BEE-INVEST | MISSION 1M", layout="wide")

st.markdown("""
<style>
    /* Réduction globale pour éviter le scroll */
    ...
    /* Ajustement des métriques Streamlit */
...
def get_market_data():
    try:
        ...
        news = sorted(feed.entries, key=lambda x: x.published_parsed, reverse=True)[:4] # Limité à 4 pour la hauteur
...
if data:
    ...
    h1.markdown(f"<h3 style='color:#ffb000; margin:0;'>🔱 BEE-INVEST UNIT</h3><small style='color:#444; font-size:8px;'>ROAD TO 1M | RISK 6%</small>", unsafe_allow_html=True)
    ...
    with col_main:
        ...
        <div class='roadmap-box'>
            <div style='display:flex; justify-content:space-between; font-size:10px;'>
                <span>Paliers : <b>{paliers_restants:.1f}</b></span>
                <span style='color:#ffb000;'>{progression:.2f}%</span>
            </div>
            <div style='background:#222; height:6px; border-radius:3px; margin-top:4px;'>
        ...
        with c_ex1:
            st.markdown(f"""<div class='kz-card' style='text-align:center; border-left: 3px solid #00ff88;'>
                <small class='label'>LOT (6%)</small><br>
                <span style='font-size:36px; font-weight:900; color:#00ff88;'>{lot_size:.2f}</span><br>
                <b style='font-size:11px; color:{'#00ff88' if can_trade else '#ffb000'};'>{'ACHAT VALIDÉ' if can_trade else 'WAIT'}</b>
            </div>""", unsafe_allow_html=True)
        with c_ex2:
            st.markdown(f"""<div class='kz-card' style='font-size:11px;'>
                <small class='label'>PALIERS PRIX</small><br>
                🟢 TP : <b>{gold+20:,.1f}</b><br>
                ⚪ IN : <b>{gold:,.1f}</b><br>
                🔴 SL : <b>{gold-15:,.1f}</b>
            </div>""", unsafe_allow_html=True)
    ...
    # Footer Verdict (Visible sans scroll)
