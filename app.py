...
# 1. Configuration & Design System (V66 LOCKED)
...
<style>
    ...
    /* MATRIX ROADMAP RESTAURATION ROUGE/BLEU */
...
def sync_terminal():
    try:
        ...
        # Calendar Correction (Sanity Check for PPI/700%)
        cal_events = []
        for n in feed.entries[:25]:
            ...
            if any(x in title for x in ["PPI", "CPI", "PMI", "FED", "NFP", "RATE"]):
                ...
                nums = re.findall(r'\d+\.\d+', title) # Ne cherche que les décimaux (ex: 2.1)
